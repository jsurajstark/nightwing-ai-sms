...
class GitHubModelsProvider(LLMProvider):
    ...
    def _chat_completion(self, url: str, payload: dict, request_timeout: float) -> dict:
        ...
        if not isinstance(data, dict):
            raise LLMError(f"Unexpected GitHub Models response type: {type(data).__name__}")
        return data
        ...
        *,
        ...
        try:
            ...
            if e.response.status_code == 400 and "response_format" in payload:
                logger.warning(
                    "GitHub Models model=%s rejected response_format; retrying without JSON mode",
                    model,
                )
    ...
    def _parse_completion(self, data: dict, model: str) -> dict:
        ...
        try:
            ...
            logger.error("GitHub Models unexpected response shape model=%s: %r", model, data)
            raise LLMError(f"Unexpected GitHub Models response shape: {data!r}") from e

        content = (content or "").strip()
        if not content:
            raise LLMError(f"GitHub Models model {model!r} returned empty content")

        if finish_reason == "length":
            raise LLMError("Model response truncated (max_tokens); retry with higher limit")

        try:
            parsed = json.loads(content)
            if not isinstance(parsed, dict):
                raise LLMError(f"Expected JSON object, got {type(parsed).__name__}")
            logger.info("GitHub Models JSON parsed model=%s keys=%s", model, list(parsed.keys()))
            return parsed
    ...
    def extract_referral(
        ...
        *,
            ...
            for attempt in range(self._max_retries + 1):
                ...
                try:
                    ...
                    if str(e).startswith("invalid_json:"):
                        raise LLMError(
                            f"Model did not return valid JSON: {str(e)[len('invalid_json:'):]}"
                        ) from e
                    raise
        ...
        raise LLMError(
            f"GitHub Models failed on all models {self._model_chain!r} after {elapsed_ms:.0f}ms: {summary}"
        )
...
def _format_http_error(e: httpx.HTTPStatusError, model: str) -> str:
    ...
    if status == 429:
        ...
        return detail
    return f"GitHub Models HTTP {status} for model {model!r}: {e.response.text[:300]}"
